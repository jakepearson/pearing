...
function math(
  ...
  if (operation[0] === "1") {
    throw new Error(`Write parameter cannot be immediate mode: ${operation}`);
  }

  const outputLocal = f(operand1, operand2);
  const outputIndex = get(offset + 3, "1", data);

  set(outputLocal, outputIndex, data);
}
...
function input(operation: string, offset: number, data: number[]) {
  ...
  const localInput = globalInputs.shift().toString(); // readline.question("Input please\n")
  set(Number.parseInt(localInput, 10), operand1, data);
}

function output(operation: string, offset: number, data: number[]) {
  const operand1 = get(offset + 1, operation[2], data);
  globalOffset = operand1;
...
function parse(input: string): number[] {
  return input.split(",").map((s) => Number.parseInt(s, 10));
}
